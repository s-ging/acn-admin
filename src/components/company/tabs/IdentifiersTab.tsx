...
function WireCodeSearchPanel({ title, searchFn, assignedCodes, onSave, onCancel }: {
  title: string
  searchFn: (q: string) => WireCodeItem[]
  assignedCodes: Set<string>
  onSave: (selected: WireCodeItem[]) => void
  onCancel: () => void
}) {
  ...
  const handleSave = () => {
    const all = searchFn('')
    onSave(all.filter(c => selected.has(c.code)))
  }
    ...
    <div className="contact-card" style={{ marginBottom: 16 }}>
      ...
      <div className="wire-search">
        ...
        <div className="wire-search__results">
          {results.map(c => {
            const isAssigned = assignedCodes.has(c.code)
            const isSelected = selected.has(c.code)
            return (
              <div key={c.code} className="wire-search__row" style={isAssigned ? { opacity: 0.45 } : undefined}>
                <span style={{ fontFamily: 'monospace', fontWeight: 500, width: 80, flexShrink: 0 }}>{c.code}</span>
                <span className="wire-search__meta" style={{ flex: 1 }}>{c.description}</span>
                <span className="wire-search__meta" style={{ fontSize: '0.72rem', width: 140, textAlign: 'right', flexShrink: 0 }}>{c.category}</span>
                <Button
...
const IdentifiersTab = memo(function IdentifiersTab() {
  ...
  const handleSaveReuters = useCallback((selected: WireCodeItem[]) => {
    if (!draft) return
  ...
  const handleSaveBloomberg = useCallback((selected: WireCodeItem[]) => {
    if (!draft) return
    const existingCodes = new Set(
  ...
  const assignedReutersCodes = new Set(reutersCodes.map(w => w.wire_code.code))
  const assignedBloombergCodes = new Set(bloombergCodes.map(w => w.wire_code.code))
    ...
    <div className="tab-content">
      ...
      {addingReuters && (
        <WireCodeSearchPanel
          ...
          searchFn={searchReutersCodes}
          assignedCodes={assignedReutersCodes}
      ...
      {addingBloomberg && (
        <WireCodeSearchPanel
          ...
          searchFn={searchBloombergCodes}
          assignedCodes={assignedBloombergCodes}
